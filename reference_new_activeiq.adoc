...
summary: 了解 Digital Advisor 的新功能和增强功能。 
---
...
* "容量添加"功能现在称为"存储扩展"。了解有关 link:https://docs.netapp.com/us-en/active-iq/concept_getting_started_2.html["Digital Advisor 小工具"] 的信息。
...
E 系列和StorageGRID系统现在支持可持续性。您可以从这些系统的可持续性仪表板查看建议操作和环境指标的列表，其中显示了电力、直接碳使用量和热量的预测。要了解更多信息，请参阅link:https://docs.netapp.com/us-en/active-iq/sustainability-dashboard-overview.html["Sustainability信息板概述"^]。
...
* 健康工作流程现在包括可持续发展小部件，该小部件提供客户、监视列表、站点和组级别的建议操作计数。您可以在可持续发展控制面板上选择操作数量，以详细查看这些建议操作。要了解更多信息，请参阅 link:https://docs.netapp.com/us-en/active-iq/learn-about-sustainability.html["分析存储系统的可持续性"]。
...
* 切换开关*无快照备份的节省*已重命名为*使用快照副本*，并更改了其功能。link:https://docs.netapp.com/us-en/active-iq/reference_aiq_faq.html#storage-efficiency["了解更多信息。"]。
...
可持续性仪表板为您的存储系统的环境可持续性提供了宝贵的见解。您可以查看可持续性得分、碳减排百分比、预计电力使用量、直接碳排放和热量等信息。您可以调整特定站点的碳减排百分比。您还可以在集群级别查看可持续性分数。根据可持续性评分，您可以评估存储系统的整体效率，并使其与 NetApp 建议的措施保持一致，以提高可持续性。link:https://docs.netapp.com/us-en/active-iq/learn-about-sustainability.html["了解更多信息。"] 。
...
SAN 和 NAS 存储效率在节点级别适用于 ONTAP 系统，包括 AFF A-Series、AFF C190、All SAN Array 和 FAS500。
...
=== Digital Advisor 与 NetApp Console 集成

Active IQ Digital Advisor 正在更改为 Digital Advisor，现已集成到 NetApp Console 中，NetApp Console 是用于混合多云环境的统一管理控制台。link:https://docs.netapp.com/us-en/active-iq/digital-advisor-integration-with-console.html["了解更多信息。"].
...
NetApp Console 用户现在可以在适用的情况下在新选项卡中打开 Digital Advisor 链接，与 Digital Advisor 中的现有功能类似。
...
* 在健康评估电子邮件中，您可以找到有关勒索软件防御的信息，而不是续订。
...
在 SVM 表中选择蓝色"Volume Summary"框后，将弹出一个"弹出窗口"，显示托管或连接到特定序列号或物理节点的卷的详细信息。
...
* * 规划小工具： *
+
** *容量增加：* 对于任何超过现有容量 70% 阈值的 GRID 站点，您将收到通知。如果容量阈值可能超过 70%，您可以选择在接下来的 1、3 和 6 个月内为站点中的 StorageGRID 添加容量。
** *续订：*对于许可证合同已过期或即将在未来 6 个月内过期的任何 StorageGRID 系统，您将收到通知。您可以选择一个或多个系统，向 NetApp 支持团队提出续订申请。


* * 网格信息板： * 网格信息板提供选定网格的运行状况，规划和配置详细信息。
* * 配置小工具： * 提供小工具中选定 StorageGRID 的基本详细信息，例如网格名称，主机名，序列号，型号，操作系统版本，客户名称，发货位置和联系详细信息。
* *GRID Viewer：* 在*配置*小部件中，您可以通过选择 *GRID Viewer* 链接详细查看 GRID 配置。从*配置*小部件中，您可以通过选择 *Grid Viewer* 屏幕中的*下载*按钮，下载选定 StorageGRID 的站点详细信息和容量详细信息。
* * 站点详细信息： * 此选项卡提供了每个站点可用的网格摘要和存储节点。
* *GRID 摘要：*包含基本信息，例如许可证类型、许可证容量、安装的节点数量、支持期限（许可证合同终止日期）、主管理节点和主管理节点的主站点。此选项卡还提供站点名称和对应站点下标记的存储节点数。在此版本中，您可以通过选择相应站点的存储节点查看超链接来查看节点名称列表。
...
Tier Data 按钮已添加到 FabricPool 控制面板，您可以使用 NetApp Console 将数据分层到低成本的对象存储层。
...
您可以使用 Digital Advisor API 提取感兴趣的数据，并将其直接集成到公司的工作流程中。link:https://docs.netapp.com/us-en/active-iq/concept_overview_API_service.html["了解更多信息。"]。
...
您可以查看 AFF 系统、非 AFF 系统或两者的存储效率比率和节省量（包含和不含快照副本）。您可以在节点级别查看存储效率信息。link:https://docs.netapp.com/us-en/active-iq/task_analyze_storage_efficiency.html["了解如何操作"]。
