...
首次显示图表时，将预先选择一个为期 1 周的选项卡，并以图形格式显示为期 1 周的数据，以便更容易理解大量数据及其在不同数据系列之间的关系。如果要重置日期范围，例如，可以选择 1 个月选项卡并选择日历中的日期。

您还可以选择放大性能图；此时将显示各个数据点。

video::fWrHYX17xT8[youtube,width=848,height=480]
.步骤
. 在控制面板上，选择 *Performance*。
+
对于 ONTAP 系统，您可以选择 *节点* 选项卡以查看 ONTAP 集群的单个节点的性能，选择 *本地层* 选项卡以查看本地层的性能，或选择 *卷* 选项卡以查看卷的性能。默认情况下，将显示集群性能。
