...
---
= 在Digital Advisor中为单个和多个集群生成ONTAP升级计划
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
* 确保已启用并正确配置 AutoSupport (ASUP)，并包含所有必需部分。有关详细信息，请参见 link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/ONTAP_AutoSupport_Resolution_Guide["ONTAPAutoSupport解决方案指南"]。
* 验证 AutoSupport 集群成员与 SAP 集群配置是否匹配。更多信息，link:https://kb.netapp.com/Support/NSS/Support_Site/How_to_open_a_non-technical_case["提交一个非技术性案例"] 。
...
.单个集群
--
. 在仪表板上，选择 *Upgrade Advisor* 小部件中的集群数量。
+
...
. 选择要升级的集群。*目标操作系统版本*列显示推荐的目标操作系统版本。您可以选择 image:edit_icon.png["\"编辑\"按钮的图像"] 图标来选择集群的另一个目标操作系统版本。
+
image:r_ua_select_target_OS_version_single_cluster.png["显示如何选择目标版本的图像。"]

+
** 您可以在*节点*列中选择节点数，以查看集群的节点摘要。
** 您可以从*建议操作*列中选择*解决问题*，以解决不合格集群的问题，使其符合升级条件。+ image:r_ua_resolve_issue.png["显示问题描述类型和修复所需分辨率的图像。"]


. 选择*生成升级计划*。您将重定向至*升级建议*页面。
...
** 您可以选择 image:ua_export_icon.png["导出按钮"] 图标将风险摘要导出到 Excel 工作表。
...
TIP: 您可以选择 *View Upgrade Advisor Reports* 转到 *Reports* 页面。+ image:r_ua_view_reports.png["显示 View Upgrade Advisor 所在位置以供选择的屏幕截图。"]
...
. 选择要升级的集群。*目标操作系统版本*列显示推荐的目标操作系统版本。您可以选择 image:edit_icon.png["\"编辑\"按钮的图像"] 图标以选择集群的另一个目标操作系统版本。
+
image:r_ua_select_target_OS_version.png["显示如何选择目标版本的图像。"]

+
** 您可以在*节点*列中选择节点数，以查看集群的节点摘要。
** 您可以从*建议操作*列中选择*解决问题*，以解决不符合条件的集群的问题，使其有资格升级。+ image:r_ua_resolve_issue.png["显示问题描述类型和修复所需分辨率的图像。"]


. 选择*Generate Upgrade Plan*。
. 在显示的弹出窗口中提供详细信息。
  +
