---
sidebar: sidebar 
permalink: task_subscribe_to_wellness_review_email.html 
keywords: wellness, wellness review subscription, unsubscribe, supportedge expert, supportedge advisor 
summary:  
---
= 订阅Digital Advisor的存储系统健康状况通知
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
:nofooter: 
:icons: font
:linkattrs: 
:imagesdir: ./media/



NOTE: Wellness Review 功能已被弃用，自 2026 年 7 月起不再在 Digital Advisor 中提供。

您可以订阅健康检查电子邮件，以每月收到一封电子邮件，其中总结了健康状况、接近续订日期的存储系统以及需要升级现有 NetApp 产品的存储系统。您将收到一封健康检查电子邮件，以便查看每月摘要并针对您的存储系统采取相应措施。

您还可以选择查看、编辑、共享和删除订阅。如果您决定不接收电子邮件，也可以随时取消订阅。

启用订阅后，您应在添加订阅时选择开始日期。每月电子邮件摘要提供了有关未完成的健康，续订，升级和运行状况检查操作的视图。您可以确认电子邮件地址，并将此电子邮件发送到指定的电子邮件地址。您还可以选择删除订阅。


NOTE: 此功能仅可通过 NetApp SupportEdge Advisor 和 SupportEdge Expert 服务产品获得。

.步骤
. 从左侧窗格中，选择 *Wellness Review*。
. 选择*添加订阅*。
. 在 * 添加新订阅 * 对话框的 * 为订阅命名 * ， * 选择类别 * ， * 搜索客户 * 和 * 电子邮件 * 字段中提供所需信息。
. 选择*订阅*。
+
成功订阅后，您将收到一条 * 已添加订阅 * 消息。


