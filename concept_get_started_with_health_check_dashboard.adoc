...
* *AutoSupport 采用*：显示已启用 AutoSupport 的系统的数量和百分比。您还可以查看标记为"拒绝"的系统、启用了 *HTTPS* 和 *AutoSupport on Demand* 的系统，以及在过去 7 天内停止发送 AutoSupport 数据的系统的*信号丢失*情况。要查看您的健康检查分数以及安装库中系统的相关信息，请选择 *AutoSupport Adoption* 小部件。
...
* *Support & Entitlements*：显示已过期和即将在 6 至 12 个月内到期的支持合同。它显示支持终止平台、磁盘、盘架、授权合规性、待到期项目，以及平台和硬件的支持终止不适用情况。您可以查看控制面板上提供的健康检查分数，并根据按优先级排列的关键建议采取相应措施。要查看有关支持合同的详细信息，请选择 *Support & Entitlements* 小部件。您也可以使用此小部件续订支持合同。
