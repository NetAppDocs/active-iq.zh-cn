...
*如何在 Active IQ Digital Advisor 中查看存储效率？*

. 打开的主页 link:https://activeiq.netapp.com/?source=onlinedocs["Active IQ Digital Advisor"]。
...
*Active IQ Digital Advisor 是否显示所有 ONTAP 系统的比率？*
...
如果选中 *不使用 Snapshots* 复选框，Active IQ Digital Advisor 将在客户级别，站点级别，组级别，集群级别和节点级别计算效率比率（重复数据删除，数据压缩，数据缩减和克隆存储效率技术的比率）。
...
*如何在 Active IQ Digital Advisor 中计算容量？*Active IQ Digital Advisor 中的容量是为集群和节点计算的，不包括根副本和 Snapshot 副本
...
*Active IQ Digital Advisor 显示的容量是二进制还是十进制？*Active IQ 中显示的所有容量均为二进制数字（除以 1024），并以 GiB/TiB 表示容量。ONTAP 存储和其他 NetApp 产品还会在 Base 2 中显示容量使用情况。
